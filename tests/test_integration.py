...
class MockWebSocketSession:
    ...
    async def receive_text(self):
        return json.dumps({"type": "content", "data": "test message", "encrypted": True})
...
async def test_shared_key_for_message_exchange():
    ...
    assert client2_decrypted == original_message, "Client 2 should be able to decrypt messages from client 1"
    ...
    response = WebSocketMessage(
        type="content",
        data=client2_encrypted,
        encrypted=True
    ).model_dump(exclude_none=True)
    
    # Client 1 decrypts
    client1_decrypted = base64.b64decode(response["data"].encode()).decode()
    assert client1_decrypted == response_message, "Client 1 should be able to decrypt messages from client 2"
