...
async def test_certificate_verification():
    ...
    response = client.post("/verify-cert", json=verification_data.model_dump())
...
async def test_key_exchange():
    ...
    key_request = KeyExchangeRequest(
        token="test_token", clientKey=base64.b64encode(client_public_bytes).decode()
    )

    # Send request
    response = client.post("/key-exchange", json=key_request.model_dump())
...
async def test_tampering_report():
    ...
    # Send report
    response = client.post("/tampering-report", json=report.model_dump())
